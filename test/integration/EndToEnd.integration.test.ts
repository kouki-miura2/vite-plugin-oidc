...
describe('End-to-End OIDC Authorization Code Flow + PKCE Integration Tests', () => {
  ...
  describe('Complete OIDC Authorization Code Flow + PKCE', () => {
    ...
    it('should complete OIDC discovery', async () => {
      ...
      const discoveryResponse = await makeRequest('GET', '/.well-known/openid-configuration');
    ...
    it('should complete authorization flow steps individually', async () => {
      ...
      const { codeVerifier, codeChallenge } = PKCETestHelper.generateValidPKCEPair();
      ...
      const authUrl = `/authorize?client_id=${clientId}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=code&code_challenge=${codeChallenge}&code_challenge_method=S256&state=${state}&scope=${encodeURIComponent(scope)}&nonce=${nonce}`;
      ...
      const loginPageUrl = authResponse.headers['Location']!.replace('/oidc', '');
      const loginPageResponse = await makeRequest('GET', loginPageUrl);
      
      expect(loginPageResponse.statusCode).toBe(200);
      expect(loginPageResponse.headers['Content-Type']).toBe('text/html; charset=utf-8');
      ...
      const loginFormData = `username=johndoe&password=password123&return_to=${encodeURIComponent(authUrl)}`;
      const loginSubmitResponse = await makeRequest('POST', '/login', {
        'Content-Type': 'application/x-www-form-urlencoded'
      }, loginFormData);
      
      expect(loginSubmitResponse.statusCode).toBe(302);
      expect(loginSubmitResponse.headers['Set-Cookie']).toContain('oidc_session=');
    ...
    it('should handle token endpoint validation correctly', async () => {
      ...
      const tokenRequestBody = `grant_type=authorization_code&code=invalid_code&redirect_uri=${encodeURIComponent(redirectUri)}&client_id=${clientId}&code_verifier=${codeVerifier}`;
      const tokenResponse = await makeRequest('POST', '/token', {
        'Content-Type': 'application/x-www-form-urlencoded'
      }, tokenRequestBody);
      ...
      const invalidGrantResponse = await makeRequest('POST', '/token', {
        'Content-Type': 'application/x-www-form-urlencoded'
    ...
    it('should handle invalid client credentials', async () => {
      ...
      const authUrl = `/authorize?client_id=${invalidClientId}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=code&code_challenge=${codeChallenge}&code_challenge_method=S256&state=test_invalid_client&scope=openid`;
      const authResponse = await makeRequest('GET', authUrl);
      
      expect(authResponse.statusCode).toBe(302);
      expect(authResponse.headers['Location']).toContain('error=unauthorized_client');
    });
  ...
  describe('Multiple User Account Testing', () => {
    it('should display different user accounts in login page', async () => {
      ...
      const authUrl = `/authorize?client_id=${clientId}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=code&code_challenge=${codeChallenge}&code_challenge_method=S256&state=test_users&scope=openid`;
      const authResponse = await makeRequest('GET', authUrl);
    ...
    it('should authenticate different users correctly', async () => {
      ...
      for (const user of users) {
        const returnTo = '/authorize?client_id=test_client&redirect_uri=http%3A%2F%2Flocalhost%3A3000%2Fcallback&response_type=code&code_challenge=test&code_challenge_method=S256&state=test&scope=openid';
        const loginFormData = `username=${user.username}&password=${user.password}&return_to=${encodeURIComponent(returnTo)}`;
    ...
    it('should reject invalid user credentials', async () => {
      ...
      const invalidUserData = `username=invalid_user&password=admin123&return_to=${encodeURIComponent(returnTo)}`;
      const invalidUserResponse = await makeRequest('POST', '/login', {
        'Content-Type': 'application/x-www-form-urlencoded'
      }, invalidUserData);
      
      expect(invalidUserResponse.statusCode).toBe(302);
      expect(invalidUserResponse.headers['Location']).toContain('/login?error=');
      expect(invalidUserResponse.headers['Location']).toContain('Invalid%20username%20or%20password');

      // Test with invalid password
      const invalidPasswordData = `username=johndoe&password=wrongpassword&return_to=${encodeURIComponent(returnTo)}`;
      const invalidPasswordResponse = await makeRequest('POST', '/login', {
        'Content-Type': 'application/x-www-form-urlencoded'
      }, invalidPasswordData);
      
      expect(invalidPasswordResponse.statusCode).toBe(302);
      expect(invalidPasswordResponse.headers['Location']).toContain('/login?error=');
      expect(invalidPasswordResponse.headers['Location']).toContain('Invalid%20username%20or%20password');
    });
  });
});