...
describe('TokenService', () => {
  ...
  describe('Constructor', () => {
    ...
    it('should use custom expiration times', () => {
      const customService = new TokenService(testConfig, issuer, {
        accessToken: 7200,
        idToken: 1800
      });
  ...
  describe('Token Validation', () => {
    ...
    it('should reject token with wrong issuer', () => {
      const wrongService = new TokenService(testConfig, 'http://wrong-issuer.com');
  ...
  describe('ID Token Claims Building', () => {
    ...
    it('should include profile claims with profile scope', () => {
      ...
      const decoded = JSON.parse(Buffer.from(response.id_token!.split('.')[1], 'base64').toString());
