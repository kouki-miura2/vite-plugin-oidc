...
async function checkAuthenticationStatus() {
    ...
    if (keycloak.token && !keycloak.authenticated) {
        console.log('Have token but not marked as authenticated, trying to validate...');
        try {
            // Try to refresh/validate the token
            const refreshed = await keycloak.updateToken(5);
            console.log('Token validation result:', refreshed, 'authenticated:', keycloak.authenticated);
    ...
    if (storedToken && !keycloak.token) {
        console.log('Found stored token but keycloak didnt load it - this shouldnt happen');
    }
...
function showUser() {
    ...
    if (keycloak.authenticated && (keycloak.tokenParsed || keycloak.idTokenParsed)) {
...
async function init() {
    ...
    el('login-btn').addEventListener('click', () => {
        ...
        try {
            keycloak.login({
                redirectUri: window.location.origin + import.meta.env.BASE_URL + 'index-kc-iframe.html',
            })
        } catch (error) {
    ...
    try {
        ...
        console.log('Has auth callback:', hasAuthCallback, 'Hash:', window.location.hash);
        ...
        if (hasAuthCallback) {
            ...
            if (storedToken || storedRefreshToken || storedIdToken) {
                ...
                console.log('No stored tokens found, initializing without session check');
                authenticated = await keycloak.init({
                    onLoad: 'check-sso',
                    pkceMethod: 'S256',
                    checkLoginIframe: true,  // Enable 3rd party cookie check
