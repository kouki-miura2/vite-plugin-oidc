...
export function createOIDCMiddleware(config: Required<OIDCPluginConfig>) {
    ...
    const authorizationHandler = new AuthorizationHandler(store, config, config.users, config.clients);
    ...
    const middleware = async (req: Request, res: Response, next: () => void) => {
        try {
            ...
            // Validate URL path
            if (!urlPath || urlPath.length > 1000) { // Prevent extremely long URLs
            ...
            if (knownEndpoints.includes(urlPath)) {
                ...
                res.end(JSON.stringify({
                    error: 'invalid_request',
                    error_description: `Method ${method} not allowed for ${urlPath}. Expected: ${getExpectedMethods(urlPath).join(', ')}`
                }));
                return;
            }
    ...
    (middleware as any).cleanup = () => {
        try {
            ...
            logger.error('Error during middleware cleanup:', {
                errorMessage: error instanceof Error ? error.message : String(error),
                timestamp: Date.now()
            }, error instanceof Error ? error : new Error(String(error)));
        }
