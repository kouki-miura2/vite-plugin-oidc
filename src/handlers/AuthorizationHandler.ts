...
import type { InMemoryStore, AuthorizationCode } from '../types/storage.js'
import type {
  UserAccount,
  ClientConfig,
  OIDCPluginConfig,
  ...
  async handleAuthorize(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      if (session) {
        ...
        const codeExpiration =
          Date.now() +
          (this.config.tokenExpiration?.authorizationCode || 600) * 1000
