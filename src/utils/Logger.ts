...
export class Logger {
   ...
   * Log error message
   */
  error(message: string, context?: LogContext, error?: Error | OIDCError): void {
   ...
   */
  logAuthorizationError(error: OIDCError, context: {
    clientId?: string;
    redirectUri?: string;
    requestId?: string;
  }): void {
    this.error('Authorization request failed', {
      endpoint: '/authorize',
      clientId: context.clientId,
      redirectUri: context.redirectUri,
      requestId: context.requestId,
   ...
   * Log token error
   */
  logTokenError(error: OIDCError, context: {
    clientId?: string;
    grantType?: string;
    requestId?: string;
  }): void {
   ...
   * Log userinfo error
   */
  logUserInfoError(error: OIDCError, context: {
    userId?: string;
    clientId?: string;
    requestId?: string;
  }): void {
    this.error('UserInfo request failed', {
      endpoint: '/userinfo',
      userId: context.userId,
      clientId: context.clientId,
      requestId: context.requestId,
  ...
  logProductionWarning(): void {
    this.warn('⚠️  vite-plugin-oidc is running in production mode. This plugin is intended for development only!', {
   ...
   */
  logValidationError(error: OIDCError, context: LogContext & {
    parameter?: string;
    ...
    const enhancedContext = {
      ...
      suggestion: context.suggestion || this.getSuggestionForError(error.error, context.parameter)
    };
   ...
   */
  private log(level: LogLevel, message: string, context?: LogContext, error?: Error | OIDCError): void {
